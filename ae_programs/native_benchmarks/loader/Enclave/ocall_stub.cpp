...
static int sgx_measure_init(uint64_t start, uint64_t size,uint64_t code_start,uint64_t code_size)
{
    int retv;
    sgx_status_t sgx_retv;
    if((sgx_retv = ocall_measure_init(&retv,start,size,code_start,code_size)) != SGX_SUCCESS) {
        dlog(__FUNCTION__ " FAILED!, Error code = %d\n", sgx_retv);
        abort();
    }
    return retv;
